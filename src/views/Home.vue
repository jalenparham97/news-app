<template>
  <div class="md-layout md-alignment-center" style="margin: 4em 0px;">
    ...
    <md-toolbar elevation="1" class="fixed-toolbar">
      ...
      <router-link class="md-title brand" to="/">NewsCycle</router-link>
      ...
      <div class="md-toolbar-section-end toolbar-small">
        ...
        <md-button class="md-icon-button" @click="showRightpanel = true">
    ...
    <md-drawer md-fixed :md-active.sync="showLeftpanel">
      ...
      >
        <md-button class="md-primary md-raised">
          <router-link to="/login" class="login-feed-btn">Login</router-link>
        </md-button>
      </md-empty-state>
      ...
      <md-empty-state
        ...
        md-label="Nothing in Feed"
        md-description="Anything you bookmark will be shown here"
      ></md-empty-state>
      ...
      <md-list v-else class="md-tripple-line feed-list" v-for="headline in feed" :key="headline.id">
        <md-list-item>
          ...
          <div class="md-list-item-text">
            <span>
              <a :href="headline.url" target="_black">{{ headline.title }}</a>
            </span>
    ...
    <div class="md-layout-item md-size-95">
      <div class="header-content" v-if="!user">
        ...
        <md-button
          class="md-raised header-action-btn md-accent"
          :md-ripple="false"
      ...
      <div class="categories-nav" ref="categoryNav">
        <div class="categories-container">
          <h2
            ...
            :key="i"
            class="news-category"
            :class="newsCategory.path === category ? 'highlight' : ''"
            @click="loadCategory(newsCategory.path)"
          >{{ newsCategory.name }}</h2>
        </div>
      </div>
      ...
      <md-content class="md-layout" style="background: #303030;">
        <ul
          ...
          :key="headline.id"
          class="md-layout-item md-xlarge-size-25 md-large-size-33 md-medium-size-33 md-small-size-50 md-xsmall-size-100 headline"
        >
...
export default {
  ...
  async created() {
    ...
    await this.$store.dispatch(
      "loadHeadlines",
      `https://newsapi.org/v2/top-headlines?country=${this.$store.state.country}&category=${this.$store.state.category}&pageSize=${this.pageSize}&apiKey=${env.API_KEY}`
    );
  },
  mounted() {
    window.onscroll = () => {
  ...
  watch: {
    async country() {
      ...
      await this.$store.dispatch(
        "loadHeadlines",
        `https://newsapi.org/v2/top-headlines?country=${this.country}&category=${this.category}&pageSize=${this.pageSize}&page=${this.page}&apiKey=${env.API_KEY}`
      );
    }
  },
  computed: {
  ...
  methods: {
    ...
    async loadSource(sourceId) {
      if (sourceId) {
        ...
        await this.$store.dispatch(
          "loadHeadlines",
          `https://newsapi.org/v2/top-headlines?sources=${this.source}&pageSize=${this.pageSize}&page=${this.page}&apiKey=${env.API_KEY}`
        );
      }
    },
    ...
    async search() {
      ...
      await this.$store.dispatch(
        ...
        `https://newsapi.org/v2/everything?q=${
          ...
          this.toDate
        )}&sortBy=${this.sortBy}&pageSize=${this.pageSize}&page=${
          this.page
        }&apiKey=${env.API_KEY}`
      );
    ...
    async loadMoreHeadlines() {
      if (this.query !== "") {
        ...
        await this.$store.dispatch(
          ...
          `https://newsapi.org/v2/everything?q=${
            this.query
          }&from=${this.dateToISOString(
            this.fromDate
          )}&to=${this.dateToISOString(this.toDate)}&sortBy=${
            this.sortBy
          }&pageSize=${this.pageSize}&page=${this.page}&apiKey=${env.API_KEY}`
        );
        console.log("Search");
        ...
        await this.$store.dispatch(
          "loadMoreHeadlines",
          `https://newsapi.org/v2/top-headlines?country=${this.$store.state.country}&category=${this.$store.state.category}&pageSize=${this.pageSize}&page=${this.page}&apiKey=${env.API_KEY}`
        );
      }
    },
    showDialog() {
